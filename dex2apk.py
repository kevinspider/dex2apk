...
def check_dex(path):
    ...
    for i in range(len(files)):
        if files[i].endswith(".dex"):
            flag = os.popen("jadx -s " + path + "/" + files[i]).read()
            print("flag", flag)
            if "INFO  - done" in flag:
                print("校验成功")
            else:
                print("校验失败, 删除文件" + path + "/" + files[i])
...
if __name__ == '__main__':
    ...
    # usage
    # jadx 需要加入环境变量
    # python dex2apk.py -a ./baobaoshu.apk -i com.babytree.apps.pregnancy